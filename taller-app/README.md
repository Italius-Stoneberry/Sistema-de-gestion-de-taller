...
├─ db/init.sql             # esquema de la base (se crea solo la 1ª vez)
└─ backend/
   ├─ Dockerfile
   ├─ package.json
   ├─ src/                 # API (auth, trabajos, cheques, pagos, usuarios, dashboard)
...
### 3.2 Pasos
1. **Copiar el proyecto** al NAS (por ejemplo a `/volume1/docker/taller-app`).
...
Si ya tenías la base creada de una versión anterior, aplicá la migración una sola vez:
```bash
cat db/migrations/001_ia.sql | docker exec -i taller-db psql -U <DB_USER> -d <DB_NAME>
```
(En instalaciones nuevas no hace falta: `init.sql` ya incluye los campos.)

**La capa de IA en sí (Ollama + n8n + WAHA + Whisper) es opcional** y está documentada aparte en
`README-IA.md`, con su propio `docker-compose.ai.yml` y un flujo de ejemplo en
`n8n/ejemplo-whatsapp-a-trabajo.json`. Se activa cuando el equipo tenga la RAM y la GPU listas.
